    ...
    for lex in lexemes:
        if inside_comment:
            if lex == '}' or lex == '*}':
                ...
                Tokens_List.append(token(lex, 'Token_type.Comment'))
        else:
            if lex in Comments:
                if lex != '{' and lex != '{*':
                    Tokens_List.append(token(lex, Comments[lex]))
                elif lex == '{' or lex == '{*':
                    inside_comment = True
                    Tokens_List.append(token(lex, Comments[lex]))
            elif lex.upper() in ReservedWords:
                Tokens_List.append(token(lex, ReservedWords[lex]))
            elif lex.upper() in Constants:
                Tokens_List.append(token(lex, Constants[lex]))
                # Check if the lexeme is an operator
            elif lex in ArithmeticOperators:
                Tokens_List.append(token(lex, ArithmeticOperators[lex]))
            elif lex in RelationalOperators:
                Tokens_List.append(token(lex, RelationalOperators[lex]))
                # Check if the lexeme is an identifier
            elif re.match("^[a-zA-Z][a-zA-Z0-9]*$", lex):
                Tokens_List.append(token(lex, Token_type.Identifier))
                # Check if the lexeme is a constant
            elif re.match("[-+]?\d+(\.\d+)?([eE][-+]?\d+)?", lex):
                Tokens_List.append(token(lex, Token_type.Number))
                # If none of the above, mark the lexeme as an error token
            else:
                Tokens_List.append(token(lex, Token_type.Error))
            pass
